...
import { filterSidebarLinks, hasPermission } from "./utils/permissions";
import {
	Home,
	User as NavUser,
	ShoppingBag,
...
const DefaultRoute = () => {
	...
	if (hasPermission(user.userType, permissions, "/dashboard")) {
		return <Navigate to="/dashboard" replace />;
	}

	// Find first accessible page for subadmin
	const filteredLinks = filterSidebarLinks(
		ALL_SIDEBAR_LINKS,
		user.userType,
		permissions,
	);
	...
	for (const link of filteredLinks) {
		if (link.href) {
			firstAccessibleRoute = link.href;
			break;
		}
		if (link.subItems && link.subItems.length > 0) {
			firstAccessibleRoute = link.subItems[0].href;
			break;
		}
	}

	// If we found an accessible route, redirect there
	if (firstAccessibleRoute) {
		return <Navigate to={firstAccessibleRoute} replace />;
	}

	// If no accessible routes, redirect to profile as fallback
