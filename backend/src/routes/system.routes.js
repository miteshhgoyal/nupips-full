...
import {
	...
	triggerManualSync,
} from "../jobs/syncMemberTree.cron.js";
...
const validateConfigUpdate = (req, res, next) => {
	...
	if (
		systemPercentage === undefined ||
		systemPercentage < 0 ||
		systemPercentage > 100
	) {
	...
	for (const item of uplineDistribution) {
		...
		if (
			typeof item.requiredRebateIncome !== "number" ||
			item.requiredRebateIncome < 0
		) {
			return res.status(400).json({
				success: false,
	...
	if (
		!performanceFeeFrequency ||
		!validFrequencies.includes(performanceFeeFrequency)
	) {
		return res.status(400).json({
			success: false,
	...
	if (
		...
		return res.status(400).json({
			success: false,
			message:
				"performanceFeeTime must be a string in HH:MM 24-hour format",
		});
	}
	...
	if (performanceFeeFrequency === "monthly") {
		if (
			...
			performanceFeeDates.some(
				(d) => typeof d !== "number" || d < 1 || d > 31,
			)
		) {
			return res.status(400).json({
				success: false,
				message:
					"performanceFeeDates must be an array of numbers between 1 and 31 for monthly frequency",
			});
		}
	}
		...
		if (syncEnabled) {
			...
			if (cronParts.length !== 5) {
				return res.status(400).json({
					success: false,
					message:
						"Sync frequency must be a valid cron expression (5 parts: minute hour day month weekday)",
				});
			}
			...
			if (!gtcLoginAccount || !gtcLoginAccount.trim()) {
				return res.status(400).json({
					success: false,
					message:
						"GTC login account is required when sync is enabled",
				});
			}
			...
			if (!gtcLoginPassword || !gtcLoginPassword.trim()) {
				return res.status(400).json({
					success: false,
					message:
						"GTC login password is required when sync is enabled",
				});
			}
	...
	req.validatedData = {
		...
		traderPercentage,
		maxUplineLevels,
		uplineDistribution: uplineDistribution.sort(
			(a, b) => a.level - b.level,
		),
		performanceFeeFrequency,
		performanceFeeDates:
			performanceFeeFrequency === "monthly" ? performanceFeeDates : [],
		performanceFeeTime,
...
router.put(
	...
	async (req, res) => {
		try {
			...
			if (
				autoSyncGTCMemberTree &&
				typeof autoSyncGTCMemberTree === "object"
			) {
				const existingSync =
					config.autoSyncGTCMemberTree?.toObject?.() ||
					config.autoSyncGTCMemberTree ||
					{};

				const updatedSync = {
					syncEnabled:
						autoSyncGTCMemberTree.syncEnabled !== undefined
		...
		try {
			...
			if (!config.autoSyncGTCMemberTree?.syncEnabled) {
				return res.status(400).json({
					success: false,
					message:
						"Auto sync is not enabled. Please enable it in system configuration first.",
				});
			}
			...
			if (
				...
				!syncConfig.gtcApiUrl
			) {
				return res.status(400).json({
					success: false,
					message:
						"Sync credentials are not configured. Please configure them in system settings.",
				});
			}
