			...
			if (req.files) {
				...
				for (let i = 0; i < newImages.length; i++) {
					if (newImages[i]) {
						...
						if (updatedImages[i]) {
							const publicId = updatedImages[i]
								.split("/")
								.slice(-2)
								.join("/")
								.split(".")[0];
	...
	async (req, res) => {
		try {
			...
			if (order.status === "Cancelled") {
				return res.status(400).json({
					message:
						"Cannot modify a cancelled order. The refund has already been processed.",
				});
			}
			...
			if (!validStatuses.includes(status)) {
				return res
					.status(400)
					.json({ message: "Invalid status", validStatuses });
			}
...
router.put("/order/cancel/:orderId", authenticateToken, async (req, res) => {
	try {
		...
		user.financials.totalRefunds =
			(user.financials.totalRefunds || 0) + order.amount;
...
router.get(
	...
	async (req, res) => {
		try {
			const { status, userId, startDate, endDate, limit, page } =
				req.query;
