...
router.get("/backup", async (req, res) => {
	try {
		...
		const dbName = mongoose.connection.name || "gtc_system";
		const filename = `nupips_backup_${date}_${time}.zip`;
	...
	try {
		...
		const dashboardData = {
			...
			overview: {
				...
				totalWalletBalance:
					userStats[0].totalWalletBalance[0]?.total || 0,
				newUsersToday: userStats[0].newUsersToday[0]?.count || 0,
				newUsersThisMonth:
					userStats[0].newUsersThisMonth[0]?.count || 0,
				...
				pendingDepositCount:
					depositStats[0].pendingDeposits[0]?.count || 0,
				totalWithdrawals:
					withdrawalStats[0].totalWithdrawals[0]?.total || 0,
				totalWithdrawalCount:
					withdrawalStats[0].totalWithdrawals[0]?.count || 0,
				pendingWithdrawals:
					withdrawalStats[0].pendingWithdrawals[0]?.total || 0,
				pendingWithdrawalCount:
					withdrawalStats[0].pendingWithdrawals[0]?.count || 0,
				totalOrders: orderStats[0].totalOrders[0]?.total || 0,
				totalOrderCount: orderStats[0].totalOrders[0]?.count || 0,
				netRevenue:
					(depositStats[0].totalDeposits[0]?.total || 0) -
					(withdrawalStats[0].totalWithdrawals[0]?.total || 0),
			},
			users: {
			...
			financial: {
				deposits: {
					today: depositStats[0].depositsToday[0] || {
						total: 0,
						count: 0,
					},
					thisMonth: depositStats[0].depositsThisMonth[0] || {
						total: 0,
						count: 0,
					},
				...
				withdrawals: {
					today: withdrawalStats[0].withdrawalsToday[0] || {
						total: 0,
						count: 0,
					},
					thisMonth: withdrawalStats[0].withdrawalsThisMonth[0] || {
						total: 0,
						count: 0,
					},
			...
			ecommerce: {
				orders: {
					today: orderStats[0].ordersToday[0] || {
						total: 0,
						count: 0,
					},
					thisMonth: orderStats[0].ordersThisMonth[0] || {
						total: 0,
						count: 0,
					},
			...
			incomeExpense: {
				...
				netProfit:
					(incomeExpenseStats[0].totalIncome[0]?.total || 0) -
					(incomeExpenseStats[0].totalExpense[0]?.total || 0),
...
router.patch("/withdrawals/:id", async (req, res) => {
	try {
		...
		if (
			status &&
			["completed", "processing", "rejected"].includes(status)
		) {
...
router.get("/users-with-gtc-status", async (req, res) => {
	try {
		...
		const gtcEmailsSet = new Set(
			gtcMembers.map((m) => m.email.toLowerCase().trim()),
		);
		...
		const joinedGTCCount = usersWithGTCStatus.filter(
			(u) => u.hasJoinedGTC,
		).length;
		const notJoinedGTCCount = usersWithGTCStatus.filter(
			(u) => !u.hasJoinedGTC,
		).length;
...
router.get("/users/:id/tree", async (req, res) => {
	try {
		const rootUser = await User.findById(req.params.id)
			.select(
				"name username email phone userType status walletBalance financials",
			)
			.lean();
...
router.get("/gtc-members", async (req, res) => {
	try {
		...
		const skip =
			tradingBalanceFilter === "top10" ||
			tradingBalanceFilter === "bottom10" ||
			walletBalanceFilter === "top10" ||
			walletBalanceFilter === "bottom10"
				? 0
		...
		const formattedBalanceStats = {
			withTradingBalance:
				balanceStats[0].withTradingBalance[0]?.count || 0,
			zeroTradingBalance:
				balanceStats[0].zeroTradingBalance[0]?.count || 0,
		...
		const formattedOnboardingStats = {
			onboardedWithCall:
				onboardingStats[0].onboardedWithCall[0]?.count || 0,
			onboardedWithMessage:
				onboardingStats[0].onboardedWithMessage[0]?.count || 0,
			bothOnboarded: onboardingStats[0].bothOnboarded[0]?.count || 0,
			notOnboarded: onboardingStats[0].notOnboarded[0]?.count || 0,
			partialOnboarded:
				onboardingStats[0].partialOnboarded[0]?.count || 0,
		};
...
router.patch("/gtc-members/:id/onboarding", async (req, res) => {
	try {
		const { onboardedWithCall, onboardedWithMessage, onboardingDoneBy } =
			req.body;

		const member = await GTCMember.findOne({
			$or: [{ _id: req.params.id }, { gtcUserId: req.params.id }],
		});
		...
		if (req.user.userType === "subadmin") {
			...
			if (
				...
				member.onboardingDoneBy.toString() !==
					req.user.userId.toString()
			) {
				return res.status(403).json({
					success: false,
					message:
						"You can only modify onboarding status for members you onboarded",
				});
			}
		}
		...
		if (onboardingDoneBy !== undefined) {
			...
			if (
				(onboardedWithCall || onboardedWithMessage) &&
				!member.onboardingDoneBy
			) {
	...
	try {
		...
		const availablePages = [
			...
			},
			{
				key: "withdrawals",
				name: "Withdrawals",
				description: "Manage user withdrawals",
			},
			{
				key: "system-incomes",
				name: "System Incomes",
				description: "View system income/expense",
			},
			{
				key: "products",
				name: "Products",
				description: "Manage products in marketing shop",
			},
			{
				key: "orders",
				name: "Orders",
			...
			},
			{
				key: "gtc-members",
				name: "GTC Members",
				description: "Manage GTC members",
			},
...
router.post("/subadmins", async (req, res) => {
	try {
		...
		const { name, username, email, phone, password, permissions } =
			req.body;
		...
		if (!name || !username || !email || !phone || !password) {
			return res.status(400).json({
				success: false,
				message:
					"All fields are required: name, username, email, phone, password",
			});
		}
		...
		if (permissions?.pages) {
			const invalidPages = permissions.pages.filter(
				(p) => !validPages.includes(p),
			);
