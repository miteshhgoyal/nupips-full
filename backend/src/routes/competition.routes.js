...
async function fetchBalanceInfo(memberId, authHeader) {
	try {
		...
		if (response.data.code === 200) {
			...
			const tradingBalance = (childData.mt_account || []).reduce(
				(sum, account) => {
					...
					if (currency === "USC") {
						...
						console.warn(
							`Unknown currency: ${currency} for account ${account.loginid}`,
						);
						return sum + balance;
					}
				},
			...
			return {
				...
				mtAccounts: (childData.mt_account || []).map((account) => ({
					...
					balanceUSD:
						account.currency?.toUpperCase() === "USC"
 ...
 */
async function fetchSelfTradingProfit(
	memberId,
	authHeader,
	competition = null,
) {
	...
	try {
		...
		if (competition) {
			payload.start_date = new Date(competition.startDate)
				.toISOString()
				.split("T")[0];
			payload.end_date = new Date(competition.endDate)
				.toISOString()
				.split("T")[0];
		}
		...
		if (response.data.code === 200) {
			...
			if (pnlData.mt_trade) {
				...
				// Calculate volume
				if (
					pnlData.mt_trade.list &&
					Array.isArray(pnlData.mt_trade.list)
				) {
					totalVolumeLots = pnlData.mt_trade.list.reduce(
						(sum, trade) => {
	...
	try {
		...
		if (response.data.code === 200) {
			...
			if (data.summary) {
				...
				pammTotalInvestment = parseFloat(
					data.summary.total_investment || 0,
				);
			}
...
async function fetchGTCData(user, competition = null) {
	try {
		...
		if (accountResponse.data.code !== 200) {
			console.warn(
				`Failed to fetch GTC account for user ${user.username}`,
			);
			return null;
		}
		...
		const selfTradingData = await fetchSelfTradingProfit(
			memberId,
			authHeader,
			competition,
		);
		...
		// 4. Fetch PAMM Profit
		const pammData = await fetchPAMMProfit(
			memberId,
			authHeader,
			competition,
		);
		...
		const profitPercent =
			totalGTCBalance > 0 ? (netProfit / totalGTCBalance) * 100 : 0;
		const winRate =
		...
		return {
			...
			isAgent:
				accountData.user_type === "agent" || accountData.is_agent === 1,
...
function calculateGrowthMetrics(user, gtcData, watermark, competition) {
	...
	// Current values
	const currentNupipsReferrals =
		user.referralDetails?.totalDirectReferrals || 0;
	...
	const nupipsReferralsGrowth = Math.max(
		0,
		currentNupipsReferrals - watermark.nupipsDirectReferrals,
	);
	const gtcReferralsGrowth = Math.max(
		0,
		currentGtcReferrals - watermark.gtcDirectMembers,
	);

	growth.directReferralsGrowth =
		competition.rules.dataSource?.directReferrals === "nupips"
			? nupipsReferralsGrowth
			: competition.rules.dataSource?.directReferrals === "gtc"
				? gtcReferralsGrowth
				: Math.max(nupipsReferralsGrowth, gtcReferralsGrowth); // 'max'

	// Team size growth
	const nupipsTeamGrowth = Math.max(
		0,
		currentNupipsTeam - watermark.nupipsTeamSize,
	);
	const gtcTeamGrowth = Math.max(0, currentGtcTeam - watermark.gtcTeamSize);

	growth.teamSizeGrowth =
		competition.rules.dataSource?.teamSize === "nupips"
			? nupipsTeamGrowth
			: competition.rules.dataSource?.teamSize === "gtc"
				? gtcTeamGrowth
				: Math.max(nupipsTeamGrowth, gtcTeamGrowth); // 'max'

	// Trading volume growth (GTC only)
	const currentVolume = gtcData?.totalVolumeLots || 0;
	growth.tradingVolumeGrowth = Math.max(
		0,
		currentVolume - watermark.gtcTradingVolumeLots,
	);
	...
	const watermarkNetProfit =
		watermark.gtcTotalProfit - watermark.gtcTotalLoss;
	...
	growth.kycCountGrowth = Math.max(
		0,
		currentKycCount - watermark.kycCompletedCount,
	);
 ...
 */
function calculateCompetitionScoreWithGrowth(
	user,
	gtcData,
	growth,
	competition,
	...
	scores.directReferrals =
		Math.min(
			growth.directReferralsGrowth /
				normalizationTargets.directReferralsTarget,
			1,
	...
	const profitScore =
		growth.profitGrowth > 0
			? Math.min(
					growth.profitGrowth /
						normalizationTargets.profitPercentTarget,
					1,
				)
			: 0;
	...
	scores.accountBalance =
		Math.min(
			Math.max(0, growth.accountBalanceGrowth) /
				normalizationTargets.accountBalanceTarget,
			1,
	...
	return {
		...
		growth: {
			...
			tradingVolumeGrowth: parseFloat(
				growth.tradingVolumeGrowth.toFixed(2),
			),
			tradingVolumeGrowthDollars: parseFloat(
				(growth.tradingVolumeGrowth * 100000).toFixed(2),
			),
			profitGrowth: parseFloat(growth.profitGrowth.toFixed(2)),
			accountBalanceGrowth: parseFloat(
				growth.accountBalanceGrowth.toFixed(2),
			),
		...
		metrics: {
			...
			gtcDirectMembers: gtcData?.gtcDirectMembers || 0,
			gtcTeamSize: gtcData?.gtcTeamSize || 0,
			tradingVolumeLots: parseFloat(
				(gtcData?.totalVolumeLots || 0).toFixed(2),
			),
			netProfit: parseFloat((gtcData?.netProfit || 0).toFixed(2)),
			selfTradingProfit: parseFloat(
				(gtcData?.selfTradingProfit || 0).toFixed(2),
			),
			...
			accountBalance: parseFloat(
				(gtcData?.totalGTCBalance || 0).toFixed(2),
			),
			kycCompletedCount: gtcData?.kycCompletedCount || 0,
...
router.get("/list", authenticateToken, async (req, res) => {
	try {
		...
		const user = await User.findById(userId).select("gtcfx").lean();
		...
		const enrichedCompetitions = competitions.map((comp) => ({
			...
			requiresConnection:
				comp.requirements?.requiresGTCAccount && !hasGTCAccount,
	...
	try {
		...
		if (competition.requirements?.requiresGTCAccount && !hasGTCAccount) {
			return res.status(403).json({
				success: false,
				message:
					"GTC account connection required to view this leaderboard",
		...
		if (userParticipant) {
			userRank = {
				...
				eligibleReward: getRewardForRank(
...
router.post(
	...
	async (req, res) => {
		try {
			...
			const participationCheck = competition.canUserParticipate(
				user,
				gtcData,
			);
			...
			if (!existingParticipant) {
				...
				watermark = await calculateHighWatermark(
					user,
					gtcData,
					competition,
				);
				...
				scoreData = calculateCompetitionScoreWithGrowth(
					user,
					gtcData,
					zeroGrowth,
					competition,
	...
	try {
		...
		if (!participant) {
			return res.json({
				...
				participating: false,
				message:
					"You have not participated in this competition yet. Calculate your score to participate.",
				competition: {
					title: competition.title,
					slug: competition.slug,
					status: competition.status,
...
router.post(
	...
	async (req, res) => {
		try {
			...
			const slug =
				competitionData.slug || generateSlug(competitionData.title);
			...
			if (competitionData.rules) {
				...
				competitionData.requirements.requiresGTCAccount =
					gtcRelatedWeight > 0 || kycWeight > 0;
			}
...
router.put("/admin/:id", authenticateToken, requireAdmin, async (req, res) => {
	try {
		...
		if (updateData.rules || updateData.kycConfig) {
			...
			updateData.requirements.requiresGTCAccount =
				gtcRelatedWeight > 0 || kycWeight > 0;
		}
		...
		try {
			...
			if (!competition.isActive()) {
				return res.status(400).json({
					success: false,
					message:
						"Can only recalculate scores for active competitions",
				});
			}
			...
			for (const participant of competition.participants) {
				try {
					const user = await User.findById(participant.userId)
						.select(
							"name username email gtcfx referralDetails userType",
						)
						.lean();
					...
					const scoreData = calculateCompetitionScoreWithGrowth(
						user,
						gtcData,
						growth,
						competition,
					...
					console.error(
						`Error processing participant ${participant.username}:`,
						error.message,
					);
					errorCount++;
				}
