...
router.delete('/admin/courses/:id',
    ...
    async (req, res) => {
        try {
            ...
            await Promise.all(
                course.videos.map(v =>
                    cloudinary.uploader.destroy(v.publicId, { resource_type: 'video' })
                )
            );
...
// Add video to course
router.post('/admin/courses/:id/videos/add',
    authenticateToken,
    requireAdmin,
    upload.single('video'),
    async (req, res) => {
        try {
            ...
            course.totalDuration = course.videos.reduce((acc, v) => acc + (v.duration || 0), 0);
...
router.put('/admin/courses/:courseId/videos/:videoId',
    authenticateToken,
    requireAdmin,
    async (req, res) => {
        try {
...
router.delete('/admin/courses/:courseId/videos/:videoId',
    authenticateToken,
    requireAdmin,
    async (req, res) => {
        try {
...
router.put('/admin/courses/:id/toggle-publish',
    authenticateToken,
    requireAdmin,
    async (req, res) => {
        try {
